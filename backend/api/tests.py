...
def fall_guy(json_file_name,required_json_file_name,Error_json_data_fname):
    ...
    filtered_items = [item for item in json_data if item["paidOnly"] == "False"]
    complete_data = []
    error_data_list = [] # for empty examples
...
# its a Dataframwork issue, these data does't exist on DataFramework
# def find_err_list_data(titelslug):
#     """
#     Find errored list items data
#     """
#     try:
#         question = GetQuestion(titleSlug=titelslug).scrape()
#         time.sleep(1)
...
# for items in Error_Fetching_data_list:
#     print(items)
    # req_data = find_err_list_data(items)
#     if req_data:
#         complete_data.append(req_data)

# print("Error Data List:", error_data_list)
# print("Complete Data:", complete_data)

# Error_Fetching_data_list = ['merge-intervals', 'longest-increasing-subsequence', 'string-compression', 'maximum-length-of-repeated-subarray', 'consecutive-characters', 'neighboring-bitwise-xor', 'minimum-sum-of-values-by-dividing-array']


# req_data = find_err_list_data('merge-intervals')
# if req_data:
#         complete_data.append(req_data)
